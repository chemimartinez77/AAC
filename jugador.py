...
class Jugador:
    ...
    def plantar(self, region, log_func=None):
        ...
        if region not in self.plantaciones:  # Comprobar si la región ya fue plantada
            if self.dinero >= region.coste_plantacion:
                ...
                if log_func:
                    log_func(f"{self.nombre} ha plantado en {region.nombre}")
            else:
                if log_func:
                    log_func(f"{self.nombre} no tiene suficiente dinero para plantar en {region.nombre}\n")
        else:
            # Mensaje si la región ya fue plantada
            if log_func:
                log_func(f"{self.nombre} ya ha plantado en {region.nombre}, no se aumentará la producción\n")
    ...
    def producir(self, log_func=None):
        ...
        if self.produccion > 0:
            ...
            if log_func:
                log_func(f"{self.nombre} ha producido {self.produccion} unidades de café\n")
        else:
            if log_func:
                log_func(f"{self.nombre} no tiene capacidad de producción para producir café\n")
    ...
    def vender(self, mercado, log_func=None, unidades_vender=0, jugadores=None):
        if unidades_vender <= self.cafe_disponible:
            ...
            if log_func:
                log_func(f"{self.nombre} ha vendido {unidades_vender} unidades de café por {ganancia} dólares\n")
            ...
            if log_func:
                log_func(f"{self.nombre} no tiene suficiente café para vender {unidades_vender} unidades\n")
